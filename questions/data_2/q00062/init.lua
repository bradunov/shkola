...
for i = 1,stampa do
    x[i] = xp[i] - dim
    y[i] = dim - yp[i]
	dist[i] = lib.math.round_dec(math.sqrt (x[i]*x[i] + y[i]*y[i]),2)
end	


for j = 1, math.floor(stampa/3) do

answ[j] = " "
    for i = 1, 3 do
        answ[j] = answ[j] .. name[3*(j-1)+i] .. ": " .. lib.check_number(dist[3*(j-1)+i], 30) .. "," .. "\(\ \ \ \ \ \ \)"
	end
end

mycanvas = function()

  lib.start_canvas(250, 240, "center")
