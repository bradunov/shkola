...
mycanvas = function()
  ...
  for i = 1,6 do
      ...
      lib.add_text(w+x, (scale+3)*w+v, point[i], text_style, false, false) 	  
      x = x + 2*w + ow
  end 
  lib.add_rectangle (v, (scale+4)*w, 2*w, w+ow, yelow_style, false, false)
  lib.add_rectangle (v, (scale+5)*w+ow, 2*w, w+ow, green_style, false, false)  
  lib.add_text(v+w, (scale+5)*w-v, "A", group_style, false, false) 
  lib.add_text(v+w, (scale+6)*w+v, "B", group_style, false, false) 	  

  for i = 1,7 do
    lib.add_line(i*(2*w + ow)-v, (scale+2)*w+ow, 0, 5*w-ow, line_style, false, false)
  end
    lib.add_line(v, (scale+2)*w+ow, 0, 5*w-ow, line_style, false, false)

  for i = 1,3 do
    lib.add_line(v, (scale+2)*w+i*(w+ow)+ow, 17*w, 0, line_style, false, false)
  end
    lib.add_line(v, (scale+2)*w+ow, 17*w, 0, line_style, false, false)

  for i = 1,6 do
      numb_a = qa[i] * step
      lib.add_input(i*(2*w + ow), (scale+4)*w+v, 40, 30, lib.check_number(numb_a,15))
      numb_b = qb[i] * step
      lib.add_input(i*(2*w + ow), (scale+6)*w-v, 40, 30, lib.check_number(numb_b,15))
  end	  
