...
base = number[choice]
base1 = - base
basedec = lib.math.round_dec(1/base, 3)
cor = ""

stampa =  1 + math.random(2)

ch = math.random(stampa)
op = math.random(2)
op3 = math.random(2)
oper = operation[op]
oper3 = operation[op3]
...
for i = 1,stampa do
	if (check[i] ~= 0 and i ~= ch) then
		...
		if (i == ch) then
		    numb[i] = "(" .. lib.dec_to_str(basemod) .. ")\(^{" .. nugr[i] .. "}\) "
        else		
		    numb[i] = "(" .. basemod .. ")\(^{" .. nugr[i] .. "}\) "
        end		
	end	
end
...
if (stampa == 2) then
    ...
    if (ind == 3) then	
        if (frc == 1 and op == 2) then 
			quest = bracl .. lib.frac_start() .. numb[1] .. lib.frac_mid() .. numb[2] .. lib.frac_end() .. bracr .. "\(^{" .. nugr[3]  .. "}\) "
        else		
    ...
    if (ind == 1) then	
		...
		if (op3 == 1) then 
		    step = (nugr[2] + nugr[3]) * nugr[5] 
        else
		    step = (nugr[2] - nugr[3]) * nugr[5]
        end	
		if (op == 1) then 
		    step = nugr[1] * nugr[4] + step
        else
		    step = nugr[1] * nugr[4] - step
        end			   
	end	
    if (ind == 2) then	
	    ...
	    if (op == 1) then 
		    step = nugr[1] * nugr[4] + nugr[2]
        else
		    step = nugr[1] * nugr[4] - nugr[2]
        end	
	    if (op3 == 1) then 
		    step = step + nugr[3] * nugr[5] 
        else
		    step = step - nugr[3] * nugr[5] 
        end			 		
    end	
...
if (step < 0) then
    step = - step
    res = lib.dec_to_str(basedec)
    answ = lib.check_string(cor,10) .. res .. lib.sup_start() .. lib.check_number(step,15) .. lib.sup_end()   	
else	
    answ = lib.check_string(cor,10) .. base .. lib.sup_start() .. lib.check_number(step,15) .. lib.sup_end()  
end	