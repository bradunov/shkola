...
name_a = {"OA", "OB", "AO", "AJ", "BJ", "BO", "BA", "AT", "JA", "JM", "BE", "JB", "OT", "OE", "EA", "AE"}
name_s = {"OJ", "AB", "TE", "BA", "ET", "AM", "OE", "JT", "BM", "OT", "JE", "OM", "BT", "JO", "TJ", "AE"}	
--[[
 ...
 mycanvas = function()
  ...
  lib.start_canvas(320, 320, "center", results)  
    ...
    if (ind == 3) then
       ...
       lib.add_line(7*w+ow, 2*w+ow, -5*w, 2*w, line_style, true, false) 	
    end	
    ...
    if (ind == 5 ) then
       lib.add_line(8*w+ow, 5*w+ow, -5*w, 2*w, line_style, true, false) 
    ...
    if (ind == 7) then
       lib.add_line(8*w+ow, 5*w+ow, -w, -3*w, line_style, true, false) 
       lib.add_line(8*w+ow, 5*w+ow, 3*w, 2*w, line_style, true, false)
    end
    ...
    if (ind == 9) then
       lib.add_line(2*w+ow, 4*w+ow, w, 3*w, line_style, true, false) 
       lib.add_line(10*w+ow, 4*w+ow, -7*w, 3*w, line_style, true, false)  
    end 
	
    if (ind == 10) then
       lib.add_line(3*w+ow, 7*w+ow, 7*w, -3*w, line_style, true, false) 
       lib.add_line(7*w+ow, 2*w+ow, 3*w, 2*w, line_style, true, false)  
    end 	
