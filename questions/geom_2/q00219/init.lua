...
if (ind[1] > 1) then
	numb[ind[1]] = math.ceil(numb[ind[1]])
	if (ind[1] < 6) then
		numb[1] = numb[ind[1]] / fct[ind[1]]
    else
		numb[1] = math.sqrt(numb[ind[1]] / fct[ind[1]])
    end
	for i = 2,6 do
		if (i ~= ind[1]) then
			numb[i] = fct[i] * numb[1]
...
quest = elem[ind[1]] .. space .. edge[ind[1]] .. " = " .. numb[ind[1]]
if (ind[1] == 6) then
    quest = quest .. meas[2]
else
    quest = quest .. meas[1]
...
for i = 2,3 do	
	ans[i-1] = elem[ind[i]] .. space .. edge[ind[i]] .. " = " .. lib.check_number(numb[ind[i]])
    if (ind[i] < 6) then
		ans[i-1] = 	ans[i-1] .. meas[1]
    else
		ans[i-1] = 	ans[i-1] .. meas[2]	
    end		
end
