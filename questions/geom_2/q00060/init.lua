...
mycanvas = function(no)
  ...
  lib.start_canvas(350, 330, "center") 
           
    lib.add_straight_path(ov-ow, 2*(v+ow), {{2*v, w-v}, {w+3*ow+v, -ov-v-2*ow}, {-w-2*v-7*ow, 0} }, green_style, true, false)  
    lib.add_straight_path(ov-ow, 2*(w+v), {{w+ov, 0}, {-w-3*ow-v, -ov-v-ow}, {-2*v, 5*v} }, yelow_style, true, false) 

    lib.add_text(3*v, 2*(v+ow), "A", text_style) 
    lib.add_text(ov+v+ow, w+2*v-ow, "B", text_style)
    lib.add_text(2*(w+ow)+v, 2*(v+ow), "C", text_style) 

    lib.add_text(2*(w+ow)+v, 2*(w+v), "T", text_style) 
    lib.add_text(w-ow, w+3*v, "R", text_style)
    lib.add_text(3*v, 2*(w+v), "S", text_style) 
    ...
    lib.add_input(2*w-3*v, ov+v+ow, 70, 30, reply)		
    ...
    lib.add_input(2*ov-v, v+ow, 70, 30, reply)		
    ...
    lib.add_input(2*v-ow, ov+v+ow, 70, 30, reply)					
    ...
    lib.add_input(2*w-3*v, w+ov+v, 70, 30, reply)	
    ...
    lib.add_input(2*ov-v, 2*(w+v)+ow, 70, 30, reply)	
    ...
    lib.add_input(2*v-ow, w+ov+v, 70, 30, reply)	
    ...
    lib.add_input(ov, 3*v-ow, 70, 30, reply)
    ...
    lib.add_input(ov+v, w-v+ow, 70, 30, reply)
    ...
    lib.add_input(ov, 2*w+v-ow, 70, 30, reply)
    ...
    lib.add_input(ov+v, w+ov+ow, 70, 30, reply)
    ...
    lib.add_input(9*v, 2*w+v-ow, 70, 30, reply)	
    ...
    for i = 1,15 do
        lib.add_line(2*ow+i*v, 2*ow, 0, 16*v, grid_style, false, false)
    end
