...
max_range = 6
for i = 1,2 do
    numb[i] = 1 + math.random(max_range)
end	
...
equ[2] = quest .. " " .. lib.check_one_option_dropdown(rel, rel[1]) .. lib.check_number(numb[1],20) .. " + " .. lib.check_number(numb[2],20)
...
broj[2] = (prod*(numb[1]+numb[2]) - sum) * denom[1]	
...
if (nr < 0) then
    ...
    set = " k " .. " " .. lib.check_one_option_dropdown(rel, rel[2]) .. lib.check_number(bound_left,20) .. space .. " k " .. " " .. lib.check_one_option_dropdown(rel, rel[1]) .. lib.check_number(bound_right,20)
else
	...
	if (nr > 0) then
		if (nr <= 5) then
			for i = 1, nr do	
				set = set .. lib.check_number(bound_left+i-1,20) .. ", "  
			end	
		else
			for i = 1, 2 do
				set = set .. lib.check_number(bound_left+i-1,20) .. ", "  
				temp = temp .. lib.check_number(bound_right -3+i,20) .. ", " 		
			end	
			set = set .. temp
		end	
	end	
	set = set .. lib.check_number(bound_right,20) .. brac[4]   
end	
