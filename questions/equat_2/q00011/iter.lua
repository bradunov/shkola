...
if (index < 3) then
    if (whl[1] ~= 0) then
       ans = ans .. lib.check_number(math.floor(whl[1]),20)
    end
    if (enum_p[1] ~= 0) then
	    temp = value[1] - whl[1]
    ...
    if (whl[2] ~= 0) then
       ans = ans .. lib.check_number(math.floor(whl[2]),20)
    end
    if (enum_p[2] ~= 0) then
	    temp = value[2] - whl[2]
        condition = "is_ok = math.eq(numerator/denominator, "..tostring(temp)..");"	          
...
if (index == 3) then
	...
	reply = lib.check_number(0,15)  .. " ".. lib.check_one_option_dropdown(sign, sign[2]) .. " x " .. lib.check_one_option_dropdown(sign, sign[sg]) .. " " .. ans	
end
if (index == 4) then
