...
if (index == 5) then
	if (ind == 1) then     --[[c[1]*x = c[4] ]]--	
		...
		if (val[1] == 0) then
			...
			if (rest == 0) then
				sol[ITEM] = lib.check_number(lib.math.round_dec(valac,1))			
			else		
				condit = "is_ok = math.eq(numerator/denominator, "..tostring(valac)..");"	
    ...
    equ[ITEM] = term_left .. " = " .. term_right
        ...
        if ((ind == 1 and qst == 2 or ind == 2 and qst == 1) or (ind == 3 and qst == 4 or ind == 4 and qst == 3)) then
			...
			if (rest == 0) then
				sol[ITEM] = lib.check_number(lib.math.round_dec(value,1))			
			else		
