...
mycanvas = function(no)
  ...
  lib.add_input(w-ow, 5*ow/2, 30, 30, lib.check_string("a", 15)) 

  lib.add_straight_path(ow, ov/2+ow, {{w, 0}}, style, true, false) 
  lib.add_input(w-ow, (ov+3*ow)/2, 30, 30, lib.check_string("b", 15)) 
 
  lib.add_straight_path(7*ow, ow/2, {{0, ov+ow}}, style, true, false) 
  lib.add_input(7*ow, ov, 30, 30, lib.check_string("d", 15))
 
  lib.add_straight_path(ow, ov, {{w, -ov}}, style, true, false)  
  lib.add_input(ow, ov, 30, 30, lib.check_string("c", 15))

  lib.add_text(7*ow, ow, ".", dot_style, false, false)
  lib.add_input(7*ow, 5*ow/2, 30, 30, lib.check_string("O", 15))
  lib.add_text(ov-2*ow+2, ov/2-2, ".", dot_style, false, false)
  lib.add_input(ov-3*ow-2, (ov+3*ow)/2, 30, 30, lib.check_string("M", 15))
